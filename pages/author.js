...
export default class extends React.Component {
  static async getInitialProps({ query }) {
    ...
    return await axios
      .post(`https://graphql.cosmicjs.com/v1`, { query: gql_query })
      .then(function(response) {
        return {
          cosmic: {
            posts: _.filter(response.data.data.getObjects, post => {
              return (
                post.metadata && post.metadata.author && post.metadata.author.slug === query.slug
              );
            }),
            global: _.keyBy(
        ...
        <main className="container">
          {this.props.cosmic.posts &&
            this.props.cosmic.posts.map(post => {
              const friendly_date = new Date(post.created_at);
              ...
              return (
                <div className="card" data-href={`/${post.slug}`} key={post._id}>
                  {post.metadata.hero.imgix_url && (
                    <a
                      href={`/${post.slug}`}
                      className="blog-post-hero blog-post-hero--short"
                      style={{ backgroundImage: `url(${post.metadata.hero.imgix_url})` }}></a>
                  )}
                  <div className="card-padding">
                    ...
                    <div className="blog__author">
                      <a href={`/author/${post.metadata.author.slug}`}>
                        <div
                          ...
                          style={{
                            backgroundImage: `url(${post.metadata.author.metafields[0].imgix_url}?w=100)`
                          }}></div>
                      </a>
                    ...
                    <div
                      ...
                      dangerouslySetInnerHTML={{ __html: post.metadata.teaser }}></div>
