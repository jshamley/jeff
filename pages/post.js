...
export default class extends React.Component {
  static async getInitialProps({ query }) {
    ...
    const globals = await axios
      .post(`https://graphql.cosmicjs.com/v1`, { query: globals_query })
  ...
  render() {
    ...
    if (this.props.cosmic.post) {
      friendly_date = new Date(this.props.cosmic.post.created_at);
      ...
      <div>
        ...
        {this.props.cosmic.post && this.props.cosmic.post.metadata.hero.imgix_url && (
          <div
            ...
            style={{
              backgroundImage: `url(${this.props.cosmic.post.metadata.hero.imgix_url})`
            }}></div>
        )}
        <main className="container">
          <div className="card-padding">
            <h2 className="blog__title">
              {!this.props.cosmic.post && <div style={{ textAlign: 'center' }}>Post Not found</div>}
                ...
                <div className="blog__author">
                  <a href={`/author/${this.props.cosmic.post.metadata.author.slug}`}>
                    <div
                      ...
                      style={{
                        backgroundImage: `url(${this.props.cosmic.post.metadata.author.metafields[0].imgix_url}?w=100)`
                      }}></div>
                  </a>
                  <div className="blog__author-title">
                    by{' '}
                    <a href={`/author/${this.props.cosmic.post.metadata.author.slug}`}>
                      {this.props.cosmic.post.metadata.author.title}
                    </a>{' '}
                    on {this.props.cosmic.post.friendly_date}
                  </div>
                  <div className="clearfix"></div>
                </div>
                <div
                  className="blog__teaser droid"
                  dangerouslySetInnerHTML={{ __html: this.props.cosmic.post.content }}></div>
              </div>
            )}
          </div>
        </main>
